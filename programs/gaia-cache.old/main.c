...
int main(int argc, char **argv) {
	...
	if (g_cachepath == 0) {
		...
		if (snprintf(cachepathbuffer, sizeof(cachepathbuffer), "%s/.gaia/gecache", getenv("HOME")) >= sizeof(cachepathbuffer))
			errx(1, "cache path too long");

		if (mkdir(cachepathbuffer, 0777) != 0 && errno != EEXIST)
			err(1, "cannot create ~/.gaia/gecache");
		...
		if (stat(g_cachepath, &st) == -1)
			err(1, "cannot stat() specified cache directory");
	...
	if ((res = gecache_open(&g_gecache, g_cachepath)) != GECACHE_OK) {
		if (res == GECACHE_DB_ERROR)
			errx(1, "cannot open gaia cache: %s (code %d)\n", gecache_strerror(res), gecache_getlastdberror(&g_gecache));
		else
			errx(1, "cannot open gaia cache: %s\n", gecache_strerror(res));
	}
...
void usage() {
	...
	fprintf(stderr, "\nOptions:\n");
	fprintf(stderr, "  -c, --cache-path=PATH   path to cache (default ~/.gaia/gecache)\n");
