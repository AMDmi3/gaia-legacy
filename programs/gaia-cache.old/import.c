...
static int import_image(FTSENT *ftsent) {
	...
	/* put into cache */
	int res;
	if ((res = gecache_image_put(&g_gecache, x, y, level, import_buffer, ftsent->fts_statp->st_size)) != GECACHE_OK) {
		if (res == GECACHE_DB_ERROR)
			fprintf(stderr, "%s: cannot import: %s (code %d)\n", ftsent->fts_path, gecache_strerror(res), gecache_getlastdberror(&g_gecache));
		else
			fprintf(stderr, "%s: cannot import: %s\n", ftsent->fts_path, gecache_strerror(res));
		close(f);
		return 0;
	}
